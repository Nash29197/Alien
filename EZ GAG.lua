--[[
    N++ Hub | by 三眼怪
    Protected & Complete Version

    此版本將所有敏感的 Roblox API 調用封裝在一個名為 `Protected` 的模擬安全層中，
    使其更難被反作弊系統偵測，並提高了代碼的可維護性。
]]
...
-- 使用安全層加載 Rayfield
local Rayfield = Protected._raw.loadstring(Protected._raw.HttpGet('https://sirius.menu/rayfield' ))()
...
do
    ...
    local function loadTargetsList()
        ...
        local success, content = Protected.call(Protected._raw.HttpGet, game, url, true )
        ...
        else warn("無法從 URL 獲取 Targets 列表: ", content) end
    end
    loadTargetsList()
...
do
    ...
    local function loadShopItemsFromURL(url)
        local s, c = Protected.call(Protected._raw.HttpGet, game, url); if s and c then local f, e = Protected._raw.loadstring(c); if f then local s2, r = Protected.call(f); if s2 and typeof(r) == "table" then return r end end end; return {}
    end
...
-- ==================== 開發者工具 ====================
do
    DevelopersTab:CreateButton({ Name = "Infinite Yield", Callback = function() Protected._raw.loadstring(Protected._raw.HttpGet('https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source' ))() end })
    DevelopersTab:CreateButton({ Name = "DEX", Callback = function() Protected._raw.loadstring(Protected._raw.HttpGet('https://raw.githubusercontent.com/DarkNetworks/Infinite-Yield/refs/heads/main/dex.lua' ))() end })
    DevelopersTab:CreateButton({ Name = "SimpleSpy", Callback = function() Protected._raw.loadstring(Protected._raw.HttpGet('https://raw.githubusercontent.com/DarkNetworks/Infinite-Yield/refs/heads/main/SimpleSpyV3/main.lua' ))() end })
end
